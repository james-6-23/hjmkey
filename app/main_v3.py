#!/usr/bin/env python3
"""
HAJIMI KING V3.0 - 主程序（性能优化版）
修复了 V2 中的所有已知问题，提供更好的性能和稳定性
"""
...
# 导入核心组件
from app.core.orchestrator_v3 import OrchestratorV3
...
def print_banner():
    ...
    banner = f"""
{ascii_art}
    :: HAJIMI KING :: (v{VERSION}) - Performance Optimized V3
...
def print_config_info():
    ...
    # 验证器并发数（新增）
    validator_concurrency = config.get("VALIDATOR_CONCURRENCY", 100)
    ...
    ║  验证器并发   : {f"{validator_concurrency} 并发连接":<58} ║
    ║  密钥存储     : {plaintext:<58} ║
    ╠════════════════════════════════════════════════════════════════════════════╣
    ║                          功能状态                                            ║
    ╠════════════════════════════════════════════════════════════════════════════╣
...
def print_v3_improvements():
    """打印 V3 版本改进"""
    improvements = """
    ╔════════════════════════════════════════════════════════════════════════════╗
    ║                          V3 版本改进                                         ║
    ╠════════════════════════════════════════════════════════════════════════════╣
    ║  ✅ 修复验证器初始化问题 - Session 正确管理                                 ║
    ║  ✅ 修复参数不匹配错误 - GeminiValidationResult 兼容性                      ║
    ║  ✅ 优化 GitHub API 请求 - 添加请求限流机制                                ║
    ║  ✅ 改进错误处理 - 防止级联失败                                            ║
    ║  ✅ 优化资源管理 - 自动清理验证器资源                                      ║
    ║  ✅ 增强日志输出 - 更清晰的错误信息                                        ║
    ╚════════════════════════════════════════════════════════════════════════════╝
...
def load_queries(config) -> List[str]:
    ...
    if not queries:
        ...
        with open(queries_file, 'w', encoding='utf-8') as f:
            f.write("# HAJIMI KING V3.0 - 搜索查询配置\n")
...
async def main():
    ...
    # 打印 V3 改进
    print_v3_improvements()
    
    logger.info("🚀 正在初始化 HAJIMI KING V3.0...")
    ...
    logger.info(f"   ✅ 验证器: Gemini Validator V3 ({config.get('VALIDATOR_CONCURRENCY', 100)} 并发)")
    ...
    try:
        ...
        with shutdown_manager.managed_execution():
            # 创建高性能验证器
            logger.info("🔧 正在初始化高性能 Gemini Validator V3...")
            validator_concurrency = config.get("VALIDATOR_CONCURRENCY", 100)
            validator = create_gemini_validator(concurrency=validator_concurrency)
            logger.info(f"✅ Gemini Validator V3 已初始化 (并发数: {validator_concurrency})")
            
            # 初始化协调器 V3
            logger.info("🔧 正在初始化协调器 V3...")
            ...
            # 运行主流程
            logger.info("=" * 80)
            logger.info("🚀 开始执行主流程 (使用 Orchestrator V3)")
            logger.info("=" * 80)
    ...
    logger.info("👋 感谢使用 HAJIMI KING V3！")
    return 0
