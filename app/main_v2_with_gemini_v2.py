#!/usr/bin/env python3
"""
HAJIMI KING V2.0 - 主程序（集成 Gemini Validator V2）
使用高性能的 Gemini Validator V2 进行密钥验证
"""
...
def print_banner():
    ...
    banner = f"""
{ascii_art}
    :: HAJIMI KING :: (v{VERSION}) - With Gemini Validator V2
...
async def main():
    ...
    logger.info("🚀 正在初始化 HAJIMI KING V2.0 with Gemini Validator V2...")
    ...
    logger.info(f"   ✅ 验证器: Gemini Validator V2 ({config.get('VALIDATOR_CONCURRENCY', 100)} 并发)")
    ...
    try:
        ...
        with shutdown_manager.managed_execution():
            # 创建高性能验证器
            logger.info("🔧 正在初始化高性能 Gemini Validator V2...")
            validator_concurrency = config.get("VALIDATOR_CONCURRENCY", 100)
            validator = create_gemini_validator(concurrency=validator_concurrency)
            logger.info(f"✅ Gemini Validator V2 已初始化 (并发数: {validator_concurrency})")
            ...
            # 运行主流程
            logger.info("=" * 80)
            logger.info("🚀 开始执行主流程 (使用 Gemini Validator V2)")
            logger.info("=" * 80)
    ...
    ╚════════════════════════════════════════════════════════════════════════════╝
            ...
            if hasattr(validator, 'cleanup'):
                ...
                logger.info("✅ Gemini Validator V2 资源已清理")
