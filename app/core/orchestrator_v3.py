"""
协调器 V3 - 性能优化版
修复了 V2 中的验证器初始化问题、参数不匹配错误，并优化了 GitHub API 请求
"""
...
class OrchestratorV3:
    ...
    def __init__(self, scanner: Scanner = None, validator: KeyValidator = None):
        ...
        logger.info("=" * 60)
        logger.info("🚀 HAJIMI KING V3.0 - PERFORMANCE OPTIMIZED")
        ...
        logger.info("✅ Orchestrator V3 initialized successfully")
    ...
    async def _ensure_validator_initialized(self):
        """确保验证器已初始化（延迟初始化）"""
        if not self._validator_initialized:
            if not self.validator:
                # 创建优化的验证器
                config_service = get_config_service()
                concurrency = config_service.get("VALIDATOR_CONCURRENCY", 100)
                self.validator = OptimizedOrchestratorValidator(concurrency=concurrency)
                logger.info(f"✅ Created Gemini Validator V3 with {concurrency} concurrency")
            
            # 确保验证器正确初始化
            if hasattr(self.validator, '_ensure_initialized'):
                await self.validator._ensure_initialized()
    ...
    async def _process_single_query(self, query: str):
        ...
        # GitHub API 请求限流
        await self._rate_limit_github_request()
    ...
    async def _process_item(self, item: Dict[str, Any]):
        ...
        for val_result in validation_results:
            ...
            if val_result.is_valid:
                # 判断是否付费
                is_paid = val_result.tier and str(val_result.tier).upper() == 'PAID'
    ...
    def _save_key_to_file(self, key: str, status: KeyStatus):
        ...
        try:
            ...
            logger.debug(f"💾 Key saved to {filename}: {masked_key}")
